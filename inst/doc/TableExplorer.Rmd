...
For this example, we'll be working with [Google BigQuery](https://cloud.google.com/bigquery/). It's often helpful to have a [link to the docs](https://cloud.google.com/bigquery/what-is-bigquery) handy, and especially the [query reference](https://cloud.google.com/bigquery/query-reference).
...
In order to run queries on BigQuery tables, you must be a member of a Google Cloud Platform (GCP) project.  You can sign up for a [free trial](https://cloud.google.com/free-trial/docs/) or contact us at info@isb-cgc.org for assistance.list
...
You can find out which Google Cloud Projects you are a member of by using the list_projects function:
```{r}
list_projects()
```
...
```{r titv, fig.align="center", fig.width=10, message=FALSE, warning=FALSE, comment=NA}
