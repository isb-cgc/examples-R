...
names(clinList[[2]]) # first one is actually not found!
...
head(clinDF$ParticipantBarcode)
...
#'
## ------------------------------------------------------------------------
dataSet <- filter(sampleDF, str_detect(sampleDF$CloudStoragePath, "rsem.genes.results"))
clinSet <- filter(clinDF, clinDF$ParticipantBarcode %in% dataSet$Patient)
